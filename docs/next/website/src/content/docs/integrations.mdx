---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, Kiro CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration uninstall kiro
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kiro CLI, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Kiro CLI hook:

```bash
herdr integration install kiro
```

The hook reports Kiro's active session identity on session start, prompt submission, and turn completion. Kiro lifecycle state continues to come from Herdr's screen manifest detection. Kiro is currently a CLI-only experimental integration: install, uninstall, and status are available through `herdr integration`, but it does not appear in Settings or the generation-1 integration API.

Herdr uses `~/.kiro` by default, or `KIRO_HOME` when set. The Kiro config directory must already exist. Install writes the Herdr-owned files `hooks/herdr-agent-session.sh` and `hooks/herdr-agent-session.json` without modifying neighboring hooks. Uninstall removes only those two files. The integration is available on Linux and macOS and requires `python3`.

Herdr resumes stored Kiro sessions in their saved pane directory with `kiro-cli chat --resume-id <id>`.
